...
class TestCSVFileReplace(unittest.TestCase):
    ...
    def test_anonymize_csv_success(self):
        ...
        replaceTerms = {"John Doe": "Person A", "Jane Doe": "Person B", "XYZ company": "Company A", "ABC company": "Company B"}

        data = {'inputFile': (io.BytesIO(csv_content.encode('utf-8')), 'test.csv'), 'replaceTerms': json.dumps(replaceTerms)}
        response = self.app.post('/api/anonymize/file/csv', content_type='multipart/form-data', data=data)
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.decode('utf-8'), "Person A,Company A\nPerson B,Company B\n")
...
class TestTXTFileReplace(unittest.TestCase):
    ...
    def test_anonymize_txt_success(self):
        ...
        data = {'inputTextFile': (io.BytesIO(txt_content.encode('utf-8')), 'test.txt'), 'replaceTerms': json.dumps(replaceTerms)}
        response = self.app.post('/api/anonymize/file/txt', content_type='multipart/form-data', data=data)
